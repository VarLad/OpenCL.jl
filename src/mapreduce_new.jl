...
@inline function reduce_warp(op, val)
    ...
    offset = Int32(1)
    while offset < 32
        val = op(val, intel_shuffle_down(val, zero(val), offset))
        offset <<= 1
    end
...
@inline function reduce_group(op, val::T, neutral, shuffle::Val{true}, ::Val{maxthreads}) where {T, maxthreads}
    ...
    wid, lane  = fldmod1(get_local_size(), get_subgroup_size())
    ...
    val = if get_local_id() <= fld1(get_local_size(), get_subgroup_size())
...
function partial_mapreduce_device(f, op, neutral, maxthreads, ::Val{Rreduce},
    ...
    localDim_reduce = get_local_size() * grain
    groupIdx_reduce, groupIdx_other = fldmod1(get_group_id(), Rlen)
...
serial_mapreduce_threshold(dev) = dev.max_work_group_size * dev.max_compute_units
...
function GPUArrays.mapreducedim!(f::F, op::OP, R::WrappedCLArray{T},
    ...
    @info serial_mapreduce_threshold(cl.device()) length(Rother)
    if length(Rother) >= serial_mapreduce_threshold(cl.device())
        kernel = @opencl extensions=["SPV_INTEL_subgroups"] launch=false serial_mapreduce_kernel(f, op, init, Val(Rreduce), Val(Rother), R, A)
        wg_info = cl.work_group_info(kernel.fun, cl.device())
        local_size = min(length(Rother), wg_info.size)
        global_size = local_size * cld(length(Rreduce), local_size) * length(Rother)
        kernel(f, op, init, Val(Rreduce), Val(Rother), R, A; local_size, global_size)
        return R
    end
    ...
    dev = cl.device()
    maxthreads = min(Int(dev.max_work_group_size),
                     Int(dev.local_mem_size) ÷ sizeof(T))
    ...
    kernel = @opencl extensions=["SPV_INTEL_subgroups"] launch=false partial_mapreduce_device(f, op, init, Val(maxthreads), Val(Rreduce), Val(Rother),
    ...
    function compute_threads(kern)
        ...
        wanted_threads = shuffle ? nextwarp(kern.pipeline, length(Rreduce)) : length(Rreduce)simd_shuffle_down
    ...
    if reduce_groups == 1
        ...
        partial_kernel = @opencl extensions=["SPV_INTEL_subgroups"] launch=false partial_mapreduce_device(
